...
\begin{document}
\scriptsize \noindent Math 617 Functional Analysis (Bueler) \hfill \emph{assigned 16 March 2026; {\color{red} version 3}}
\normalsize\medskip
...
\centerline{\textbf{Due Monday 30 March at the beginning of class} {\color{red} (updated!)}}
\medskip
\normalsize
...
\prob{P16}  Suppose $\rho(t,x)=\rho(t,x_1,x_2,x_3)$, for $t\in\RR^1$ and $x\in\RR^3$, is continuously-differentiable (in each variable).  Suppose that the vector field $\bJ(t,x) = \bJ(t,x_1,x_2,x_3)$ is also continuously-differentiable.  We interpret $\rho$ as a scalar mass density (or charge density etc.) and $\bJ$ as a vector-valued mass flux.  One says that mass is conserved if
\begin{equation}
\frac{\partial\rho}{\partial t} + \Div \bJ = 0,  \label{cont}
\end{equation}
a partial differential equation (PDE), applies.  Now suppose that $\Omega\subset\RR^3$ is any open and bounded set such that $\partial\Omega$ is continuously-differentiable.  Apply the divergence theorem to \eqref{cont} to show
\begin{equation}
