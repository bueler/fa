...
\begin{document}
\scriptsize \noindent Math 617 Functional Analysis (Bueler) \hfill \emph{assigned 16 March 2026}
\normalsize\medskip
...
%plan to do sections: 5.1, 5.2, [add dual spaces], [add Riesz rep'n thm], 5.3, 6.1
%plus slides
...
\epart{c} Show the following integration-by-parts formula, justifying the existence of all (Lebesgue) integrals:
...
\epart{c} Outline how you would change the argument in \textbf{(b)} so that it would work on any open set $\Omega\subset\RR^d$, not just a cube.
...
\epart{a} Suppose $f\in L^1(\RR^1)$.  Show that $\hat f(t)$ is now well-defined for every $t$, and that $\|\hat f\|_\infty \le \frac{1}{\sqrt{2\pi}} \|f\|_1$.

\epart{b} Again suppose $f\in L^1(\RR^1)$.  Use Lebesgue's dominated convergence theorem (DCT) to show that $\hat f$ is uniformly continuous.

\epart{c} Suppose $f\in L^1(\RR^1)$, and also that $g(x)=-ixf(x)$ is in $L^1(\RR^1)$.  Use the DCT to show that $\hat f$ is differentiable at every $t$, and that $\hat f'(t) = \hat g(t)$.
