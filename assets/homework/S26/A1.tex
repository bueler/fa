...
\begin{document}
\scriptsize \noindent Math 617 Functional Analysis (Bueler) \hfill \emph{assigned 12 January 2026}
\normalsize\medskip
...
\bigskip
\noindent  \emph{Comment.}  If the $\{f_k\}$ were a ``basis'' then the values $\{\ell_k = \ell[f_k]\}$ would ``represent'' $\ell$ as an infinite-length vector.  For a finite linear combination, $g(x)=\sum_{k=1}^n c_k f_k(x)$, we do have $\ell[g] = \sum_{k=1}^n c_k \ell_k$ by linearity, and from part \textbf{(b)} this defines $\ell$ on all polynomials.  However, defining ``basis'' for infinite-dimensional spaces is quite nontrivial.  The best kind of ``basis'' is the \emph{complete orthonormal sequence}, an orthonormal basis, defined in Chapter 4, but we are not there yet; also see the footnote on page 11.  Note that Problem \textbf{P1} only requires that you figure out how to do some integrals, and you don't have to prove anything is a basis.  We will eventually see that if we change the vector space to a Hilbert space then this $\ell$ does have a representation, and indeed it is represented by the function $e^x$ itself.
