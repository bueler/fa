...
\begin{document}
\scriptsize \noindent Math 617 Functional Analysis (Bueler) \hfill \emph{assigned 16 February 2026}
\normalsize\medskip
...
\prob{P6}  \ppart{a} Consider a complex sequence $(a_n)_{n=1}^\infty$.  Show that if $a_n \to a \in \CC$ then the new sequence
	$$b_n = \frac{1}{n} \sum_{k=1}^\infty a_k,$$
...
\epart{c}  \emph{This part is a writing assignment.  You do not need to state the proofs, and no credit will be given for doing so.  Instead, explain the statement of the theorem informally, and explain its significance relative to the theory of Fourier series of continuous functions covered in lectures.}

\medskip \noindent At a young age the Hungarian mathematician Lip\'ot Fej\'er proved \emph{Fej\'er's Theorem} which shows that from the Fourier series of a continuous function one can construct a sequence of trigonmetric polynomials which converge uniformly to the continuous function.  Please look up this theorem, on the internet or in other books, and understand it.  Then explain in complete sentences, at least 5 sentences, but at most 10, what this theorem says, how it uses Cesaro means, and how it is related to what Dirichlet proved.  (Recall that Dirichlet's 1829 result is stated in the slides for weeks 4 \& 5.)
