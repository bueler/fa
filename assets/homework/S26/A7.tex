...
\bigskip
\noindent This Assignment is based on the slides, especially those for \href{https://bueler.github.io/fa/assets/slides/S26/week10.pdf}{weeks 10 \& 11} and for \href{https://bueler.github.io/fa/assets/slides/S26/week12.pdf}{week 12}.
...
\prob{P19}  Recall that the usual definition of the norm in $H^k(-\pi,\pi)$ is
	$$\|f\|_{H^k}^2 = \sum_{\ell=0}^k \int_{-\pi}^\pi |f^{(\ell)}(x)|^2\,dx,$$
where $f^{(\ell)}$ denotes the weak derivative of order $\ell$.

\epart{a}  Suppose $f\in H^1(-\pi,\pi)$.  Using the formulas for Fourier series from the week 12 slides, justify the derivative formula
	$$f'(x) = \frac{1}{\sqrt{2\pi}} \sum_{n\in\ZZ} i n c_n e^{inx},$$
where $c_n$ are the Fourier coefficients of $f$.  (\emph{Hint.  Use Lebesgue's DCT.})

\epart{b}  Show in detail, that
