...
\begin{itemize}
\item Cauchy-Schwarz (Theorem 1.3\bap)
\item arbitrary unions of open sets and intersections of closed sets (Theorem 2.2\bap)
\item compact sets in metric spaces are closed (Theorem 2.3\bap)
\item in a metric space, compactness is equivalent to sequentially compactness (Theorem 2.4\bap)
\item Heine-Borel theorem for $\RR^n$: a subset is compact if and only if it is closed and bounded (Theorem 2.5)
\item Arzel\`a-Ascoli theorem (Theorem 2.6)
\item $C([a,b]),\|\cdot\|_\infty$ is complete (Theorem 2.7)
\item 
\item FIXME
\item Bessel's inequality (Theorem FIXME)
\end{itemize}

\normalspacing
\medskip
\noindent \textbf{Techniques and facts}.  Be able to justify and use these calculation/proof techniques.
\begin{itemize}
\item Norms generate metrics (Theorem 1.1) and inner products generate norms (Theorem 1.2).
\item To show that a vector space is infinite dimensional, find a linearly-independent set with $n$ elements, for each $n\in \NN$.
\item $C([a,b]),\|\cdot\|_\infty$ and $\ell^p$ are separable (for $1\le p <\infty$).
\item $\ell^p$ is complete (for $1\le p \le \infty$).
\item FIXME
\item The norm in a normed vector space, and the inner product in an inner product space, is continuous.
\end{itemize}
