...
\bigskip
\begin{quote}
The first Midterm Exam will cover Chapters 1--4 in the textbook,\footnote{\textbook} plus any closely-related ideas in the ``calculation'' slides.\footnote{The slides are linked on the \href{https://bueler.github.io/fa/daily.html}{Daily Log tab} of the public course page.}  Specifically, the Exam will cover sections 1.1, 1.2, 1.3, 2.1, 2.2, 2.3, 3.2, 3.3, 3.6, 4.1, 4.2, and 4.3.
...
\item inner product $\ip{\cdot}{\cdot}$, and inner product space, both real and complex\footnote{In the textbook, complex inner products have the conjugate on the second variable, e.g.~$\ip{f}{g} = \int f(x)\overline{g(x)}\,dx$, and thus symmetry is $\ip{u}{v}=\overline{\ip{v}{u}}$, and linearity is in the first variable $\ip{\lambda u}{v} = \lambda \ip{u}{v}$.}
...
\item $C([a,b])$, continuous functions on $[a,b]$, real or complex, and the sup norm $\|\cdot\|_\infty$
\item $C([a,b])$ as an inner product space with $\ip{f}{g}=\int_a^b f(x) \overline{g(x)}\,dx$
\item linear algebra which needs no topology: linear independent, span, $\infty$-dimensional
\item balls in metric spaces
\item limit points, isolated points, and interior points in metric spaces
\item open and closed subsets of a metric space
\item open covers, and subcovers, in metric spaces
\item compact sets and sequentially compact sets in metric spaces
...
\item countably-additive functions on sets
\item measure
\item interval in $\RR^n$
\item $\cE$ is the collection of all finite disjoint unions of intervals
\item $2^X$ is the collection of all subsets of $X$, the power set
\item outer measure $m^*$ on arbitrary subsets of $\RR^n$
...
\item $\int_E f\,dm$, that is, the Lebesgue integral of $f$ over $E$
...
\begin{itemize}
\item Cauchy-Schwarz (Theorem 1.3; \bap \, in the real case)
\item arbitrary unions/intersections of open/closed sets (Theorem 2.2; \bap)
\item compact sets in metric spaces are closed (Theorem 2.3; \bap)
\item compactness $\iff$ sequentially compactness (Theorem 2.4; \bap)
\item Heine-Borel theorem for $\RR^n$: compact $\iff$ closed and bounded (Theorem 2.5)
\item Arzel\`a-Ascoli theorem (Theorem 2.6)
\item $C([a,b]),\|\cdot\|_\infty$ is separable (Section 2.2).
\item $C([a,b]),\|\cdot\|_\infty$ is complete (Theorem 2.7)
\item the sequence spaces $\ell^p$, for $1\le p < \infty$, are separable (\bap)
\item the sequence spaces $\ell^p$, for $1\le p \le \infty$, are complete
\item $\cM$ is a $\sigma$-ring, and $m^*$ is countably additive on $\cM$ (Theorem 3.6)
\item equivalent definitions of measurable (Theorem 3.7)
\item algebra and limits preserve measurability (Theorem 3.9)
\item approximating functions by simple functions (Theorem 3.10; \bap)
\item basic properties of the Lebesgue integral (Theorems 3.11 and 3.12)
\item for $1\le p <\infty$, $L^p(X,\mu)$ is a normed linear space (Theorems 3.17 and 3.19)
\item for $1\le p <\infty$, $L^p(X,\mu)$ is complete (Theorems 3.21)
\item Fourier coefficients are $c_k=\ip{f}{f_k}$ (Theorem 4.1; \bap)
\item Bessel's inequality (Theorem 4.2)
\item the original Riesz-Fischer theorem (Theorem 4.4)
\item the full-power Riesz-Fischer theorem from lecture, proven using the Gram-Schmidt process: separable Hilbert spaces $H$ are isometrically isomorphic to $\ell^2$
\item equivalent conditions showing that an ON sequence is complete (Theorem 4.5)
\item Fourier's sequence is complete; classical series converge in mean (Theorem 4.6)
\end{itemize}
...
\begin{itemize}
\item Norms generate metrics (Theorem 1.1), inner products generate norms (Thm 1.2).
\item To show infinite dimensional, find a linearly-independent set with $n$ elements, for each $n\in \NN$ (Section 1.3).
\item The norm in a normed vector space, and the inner product in an inner product space, are continuous.
\end{itemize}
...
\begin{enumerate}
\item On page 19, \emph{equicontinuous at $x \in [a,b]$}, for a set $E$, is defined correctly.  However, the given definition of ``equicontinuous set on an interval'' is not the correct hypothesis for Theorem 2.6.  The textbook should say:
...
\bigskip
\item On page 46 of the textbook there is a minor omission, namely of a finite-measure assumption.  (\emph{I suppose that the author wants to avoid a complicated statement.  However, without an addititional hypothesis the textbook's definition can generate $\infty-\infty$ ambiguities.})  The textbook should say:
...
\bigskip
\item The definition of a \emph{step function} on page 67 is not correct, and the proof of Theorem 3.22 should be ignored.  What the textbook says is nearly vacuous, whereas Riesz's result for Euclidean space, namely Theorem 3.22 when corrected, is significant.
...
\medskip \noindent Finally, note that the density of step functions is \emph{not} automatic for general measure spaces $(X,\cR,\mu)$; Theorem 3.22 is for Lebesgue measure.
\end{enumerate}
\end{document}
