...
\begin{center}
\alert{if $\varphi\in C_c^\infty(\Omega)$ then the integral over $K=\operatorname{supp}(D^\alpha \varphi)$, for any $\alpha$,}

\vspace{2mm}
\alert{of any continuous or integrable function, is well-defined and finite}
\end{center}
\end{itemize}
\end{frame}
...
\begin{align*}
\int_\Omega D^\alpha f\, \varphi\,dm &= - \int_\Omega D^{\tilde\alpha} f\, D^{(1,0,\dots,0)}\varphi\,dm \\
 &= \dots = (-1)^{\alpha_1} \int_\Omega D^{\tilde{\tilde\alpha}} f \, D^{(\alpha_1,0,\dots,0)}\varphi\,dm \\
 &= \dots \\
 &= (-1)^{|\alpha|} \int_\Omega f\, D^\alpha\varphi\,dm
\end{align*}
\item thus the weak derivative $g$ exists, and $g=D^\alpha f$ a.e.
\end{itemize}
\end{frame}
