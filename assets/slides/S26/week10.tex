...
\subtitle{calculations for weeks 10 \& 11 \quad (\emph{version 2})}
...
\begin{frame}{weak derivatives: Example 3 (\emph{revised})}

\begin{itemize}
\item weak derivatives can exist for unbounded functions
\item for example, consider $f\in\Lloc(\RR^2)$ given by $f(x) = \ln(r)$ where $r=|x|$
\item $f\in\Lloc(\RR^2)$ because $f$ is continuous except at $x=0$, and because this integral gives a finite answer:
	$$\int_{B_\eps} |f|\,dm = \int_{B_\eps} |\ln(r)|\,dm = - 2\pi \int_0^\eps \ln(r) \,r\,dr = \frac{\pi\eps^2}{2}(1-2\ln(\eps)) < +\infty$$

    \begin{itemize}
    \item[$\circ$] note that $dm = r\,dr\,d\theta$ in polar coordinates for $\RR^2$, where $r=|x|$
    \end{itemize}
\item the weak derivative is $g(x) = (\grad f)(x)$, namely
	$$g(x) = -\frac{\left<x_1,x_2\right>}{r^2},$$
and $g\in \Lloc(\RR^2)$ because again $g$ is continuous except at $x=0$, and
