...
Suppose $\Omega\subset\RR^2$ is bounded and open.  Suppose that $\partial \Omega$ is continuously-differentiable, and let $\bn$ be the outward normal along $\partial \Omega$.  If $\bV$ is a continuously-differentiable vector field on $\overline{\Omega}$ then
...
\begin{block}{Example}
Suppose $\Omega = \{(x,y)\,:\,x^2+y^2<1\}$ is the unit disk, so that $C=\partial\Omega$ is the unit circle.  Let $\bV=\left<x,y+1\right>$ be the vector field.  Is it true?:
	$$\int_\Omega \Div \bV\,dm \stackrel{?}{=} \int_{\partial\Omega} \bV\cdot \bn\,ds$$
\end{block}

\noindent solution steps:
\begin{itemize}
\item<2> parameterize the circle: $(x(t),y(t))=(\cos(t),\sin(t))$, $0\le t \le 2\pi$
\item<2> $\bn=\ip{x}{y}=\ip{\cos(t)}{\sin(t)}$; already normalized
\item<2> $ds=dt$ (since arclength parameterized)
...
Suppose $\Omega\subset\RR^d$ is bounded and open.  Suppose that $\partial \Omega$ is continuously-differentiable, and let $\bn$ be the outward normal along $\partial \Omega$.  If $\bV$ is a continuously-differentiable vector field on $\overline{\Omega}$ then
	$$\int_\Omega \Div \bV\,dm = \int_{\partial\Omega} \bV\cdot \bn\,ds$$
\end{theorem}
...
\begin{frame}{picture, and sketch proof, of divergence theorem in $\RR^3$}
...
Suppose $\bE:\RR^3\to\RR^3$ is the electric field.  Suppose $\Omega \subset \RR^3$ is a bounded domain with smooth boundary $\partial\Omega$.  Then
...
\begin{itemize}
\item apply the divergence theorem on the left, and write $Q$ as the integral of a charge density function $\rho:\RR^3\to\RR$:
...
\item if we apply the FTC on both sides, integrating from $a$ to $b$:
...
FIXME $\Div (f\bV) = \grad f\cdot \bV + f\Div \bV$
