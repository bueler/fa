...
\node[box, below=of UC]  (C)   {continuous at each $x$};
\draw[impl] (C1) -- (Lip);
\draw[impl] (Lip) -- (AC);
\draw[impl] (AC) -- (UC);
\draw[impl] (UC) -- (C);
...
\begin{enumerate}
\item If $f \in L^1([a,b])$ then $F(x) = \int_a^x f(t)\,dt$ is in $AC([a,b])$, and it is differentiable a.e., and $F'=f$ a.e.
...
\begin{theorem}[divergence theorem]
Suppose $\Omega\subset\RR^d$ is bounded and open, and $\partial \Omega$ is continuously-differentiable.  Let $\bn$ be the outward normal along $\partial \Omega$.  If $\bV$ is a continuously-differentiable vector field on $\overline{\Omega}$ then
	$$\int_\Omega \Div \bV\,dm = \int_{\partial\Omega} \bV\cdot \bn\,ds$$
\end{theorem}
...
\item the Fundamental Theorem of Calculus, in any dimension, says that the integral of a derivative can be computed by integrating over the boundary:
    ...
    \begin{itemize}
    \item[$\circ$] making this precise requires defining \emph{differential forms}
    \item[$\circ$] which I'm not going to do
    \end{itemize}
\item but we have this stack of Fundamental/Divergence Theorems:
\begin{align*}
...
\int_\Omega \Div \bV\,dm &= \int_{\partial\Omega} \bV\cdot \bn\,ds &&\text{\footnotesize (dim $d$ domain, dim $d-1$ closed boundary)}
\end{align*}
\end{itemize}
\end{frame}
...
\item rearranging, we get integration-by-parts:
...
\item notation:
    ...
    \end{itemize}
\item there are \alert{two} critical multi-variable product rules to know:\footnote{there are also product rules for the curl ($\nabla\times\bV$), and/or for the exterior derivative and differential-forms wedge product ($d$, $\omega\wedge\nu$); a priority if you work with electricity and magnetism}

\begin{block}{product rule for partial derivatives (scalar functions)}
    $$\frac{\partial}{\partial x_k} \Big(f \,g\Big) = \frac{\partial f}{\partial x_k} g + f \frac{\partial g}{\partial x_k}$$
\end{block}

\begin{block}{product rule for divergence}
...
\begin{itemize}
\item two integration-by-parts rules in $\RR^d$ come from the product rules \dots same logic as in $\RR^1$!
\item stating the first cleanly requires making sure the boundary term is zero, thus the assumption of compact support

\begin{block}{integration-by-parts: partial derivatives}
hypo
    $$XX\frac{\partial}{\partial x_k} \Big(f \,g\Big) = \frac{\partial f}{\partial x_k} g + f \frac{\partial g}{\partial x_k}$$
\end{block} 

\item the second formula, easily derived from the divergence theorem, is sometimes called a \emph{Green's identity}, or just a \emph{Green's theorem}

\begin{block}{integration-by-parts: divergence/gradient}
hypo
