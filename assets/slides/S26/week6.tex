...
\section{measurable subsets of $\RR^1$, and simple functions}
...
\item a measurable set can be squeezed between a closed and an open set:
	$$A \in \cM \iff \begin{matrix} \forall \eps>0\, \exists G \text{ open and } E \text{ closed s.t. } \\ E \subset A \subset G \text{ and } m^*(G\setminus E)<\eps \end{matrix}$$
\item there exist $A\subset \RR^1$ that are not Lebesgue measurable
\end{itemize}
\end{frame}
...
is the \emph{indicator function} $\one_A:\RR^1\to\RR$ for the set $A\subset \RR^1$
\end{definition}
