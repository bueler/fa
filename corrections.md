...
> **Theorem 3.13.**  Suppose that $A\in \mathcal{M}$ and that $\\left\\{f_k\\right\\}_{k=1}^\infty$ is a sequence of measurable functions such that
>  $$0 \le f_1(x) \le f_2(x) \le \cdots \quad \text{for almost all } x\in \mathcal{A}.$$
> Suppose that $\\left\\{\int_A f_k\,dm\\right\\}_{k=1}^\infty$ is a bounded set of (nonnegative) real numbers.  Let $f$ be defined to be the pointwise limit, $f(x) = \lim_{k\to\infty} f_k(x)$, of this sequence.  Then $f$ is integrable and
